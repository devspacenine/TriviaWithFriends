(function($) {
	...
	*/
    ...
    for(param in window.location.search.substr(window.location.search.indexOf('?')+1).split('&')) {
        var vals = param.split('=');
        $ite.params[vals[0]] = vals[1];
    }
	...
	*/
	...
	$.extend($ite, {
		...
		loadContent: function(url_path, targets, animations, args) {
			return $.Deferred(function(dfd){
				args = (typeof args === 'undefined') ? {} : args;
				var deferreds = $.map(targets, function($target, node_name){
					args.node_name = node_name;
					return $ite.deferredAJAX(url_path, $target, ((typeof animations[node_name] === 'undefined') ? 'fade' : animations[node_name]), args);
				});
				$.when.apply(deferreds).then(function(){dfd.resolve();});
			}).promise();
		},
		...
		changeView: function(args) {
			...
			if(_(opt.target).isString() && _(opt.node).isString() && _(opt.url).isString() && _(opt.group) && _(opt.id).isString()) {
				var group = $(opt.group),
				selected = group.filter('.selected').data('name'),
				animation = '';
				if(!_(opt.animation).isString())
					animation = opt.animation[selected];
				else
					animation = opt.animation;
				$ite.deferredAJAX(opt.url, $(opt.target), animation, {"node_name": opt.node});
				$ite.changeSelection(group, opt.id);
			}
		},
	...
	$.extend($.fn, {
        ...
        doIf: function(bool, trueCallback, falseCallback) {
            return this.each(function(){
                if((typeof bool === 'function' ? bool.call(this) : bool)) {
                    trueCallback.call(this);
                }else if(typeof falseCallback !== 'undefined') {
        ...
        /*
        * HTML element sorter similar to JavaScript's [].sort function
        */
			...
			*/
			makeHistory: function(el) {
				...
				*/
				...
				history.pushState(null, null, uri.path());
			...
			$.extend($ite, {
				...
				*/
				makeHistory: function(el) {
					...
					doc.write("<html><head><script type=\"text/javascript\">parent.$ite.onFrameLoaded('" + uri.path() + "');</script></head><body></body></html>");
					doc.close();
				},
	...
	*/
	$(function() {
		...
		$(document.body).delegate('a:not(.outbound)', 'click', function(e) {
			var target = e.target;
